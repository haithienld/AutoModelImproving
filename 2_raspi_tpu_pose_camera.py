...
def draw_pose(cv2_im,f,framcount,xgb_model_loaded, pose, src_size, inference_box, color='yellow', threshold=0.2):
    ...
    f.write(str(framcount) + "," + str(datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S.%f'))+","+str(fullkeypoint)+",squad_infrontofcamera")
    f.write('\n')
    for a, b in EDGES:
...
def main():
    ...
    #cap = cv2.VideoCapture(args.videosrc)
    ...
    #################################################################################
    out = cv2.VideoWriter(str(datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S.%f'))+'.mp4',cv2.VideoWriter_fourcc('M','J','P','G'), 10, (1280,960))
    ...
    #while cv2.waitKey(1)<0:
        ...
        cv2_im = cv2.resize(cv2_im,(1280,960))
