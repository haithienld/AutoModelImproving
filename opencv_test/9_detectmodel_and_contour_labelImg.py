...
def main():
    ...
    while cap.isOpened():
        ...
        #
        cv2.imshow("compare area", original)
        '''
...
def save(filename,
        ...
        for key, value in otherData.items():
            ...
            data[key] = value
        try:
            with open("images_img/" + filename, "w") as f:
...
def append_objs_to_img(cv2_im, inference_size, objs, labels,frame_count,check_moving):
    ...
    if check_moving == True: 
        cv2.imwrite("images_img/frame%d.jpg" % frame_count, write_image)
        #save to labelme
...
def create_xml(users_list,filename, height, width, channels):
    ...
    ET.SubElement(root, "folder").text = "images_img" 
    ...
    tree.write("images_img/"+filename +".xml",encoding='utf-8', xml_declaration=True)
