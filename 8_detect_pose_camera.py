...
def main():
    ...
    parser.add_argument('--model_pose', help='.tflite model path.', required=False)
    #=========================Detection===============================================
    parser.add_argument('--model_detection', help='.tflite model path',
                        default=os.path.join(default_model_dir,default_model_detection))
    parser.add_argument('--default_labels_detection', help='label file path',
    ...
    default_model_pose = 'models/mobilenet/posenet_mobilenet_v1_075_%d_%d_quant_decoder_edgetpu.tflite'
    if args.res == '480x360':
        src_size = (640, 480)
        appsink_size = (480, 360)
        model_pose = args.model_pose or default_model_pose % (353, 481)
    elif args.res == '640x480':
        src_size = (640, 480)
        appsink_size = (640, 480)
        model_pose = args.model_pose or default_model_pose % (481, 641)
    elif args.res == '1280x720':
        src_size = (1280, 720)
        appsink_size = (1280, 720)
        model_pose = args.model_pose or default_model_pose % (721, 1281)

    #Model Pose#
    print('Loading model: ', model_pose)
    engine = PoseEngine(model_pose)
    input_shape = engine.get_input_tensor_shape()
    inference_size = (input_shape[2], input_shape[1])
    #Model Detection#
    print('Loading {} with {} labels.'.format(args.model_detection, args.labels))
    interpreter = make_interpreter(args.model_detection)
    interpreter.allocate_tensors()
    labels = read_label_file(args.default_labels_detection)
    inference_size_detect = input_size(interpreter)
    ...
    #while cv2.waitKey(1)<0:
        ...
        cv2_im_detect = cv2.resize(cv2_im_rgb, inference_size_detect)
        run_inference(interpreter, cv2_im_detect.tobytes())
        objs = get_objects(interpreter, args.threshold)[:args.top_k]
        cv2_im_detect = append_objs_to_img(cv2_im, inference_size_detect, objs, labels)
