...
def draw_pose(cv2_im,f,framcount,xgb_model_loaded, pose, src_size, inference_box, color='yellow', threshold=0.2):
    ...
    savetotext =str(framcount) + "," + str(datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S.%f'))+","+str(fullkeypoint)+",up_down"
...
def mainthreading(cap,engine,input_shape,src_size,inference_size,model,xgb_model_loaded,framcount,f,out):
        ret, frame = cap.read()
...
def main():
    ...
    #while cv2.waitKey(1)<0:
        cv2_im1 = mainthreading(cap1,engine,input_shape,src_size,inference_size,model,xgb_model_loaded,framcount1,f,out)
        cv2_im2 = mainthreading(cap2,engine,input_shape,src_size,inference_size,model,xgb_model_loaded,framcount2,f,out)
        cv2.imshow('frame1', cv2_im1)   
        cv2.imshow('frame2', cv2_im2)   
        #==============================================================
        '''
        encoded, buffer = cv2.imencode('.jpg', cv2_im1)
        jpg_as_text = base64.b64encode(buffer)
        mystring="iiiiiiihello"
        #md = dict(dtype = str(mystring), shape=[1])
        #mess = mystring.encode('ascii')
        #jpg_as_text += base64.b64encode(mess)
        #my_ndarray = np.array([1,2,3])
        footage_socket.send(jpg_as_text)
        footage_socket.send_json(mystring)
        '''
        #send_array_and_str(footage_socket,cv2_im1,mystring)
        '''
        ret, frame = cap.read()
        #frame = cv2.rotate(frame, cv2.ROTATE_90_CLOCKWISE)
        frame = cv2.resize(frame,(640,480))
