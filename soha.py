...
''' run with image 
python3 social-distancing-faster.py --image enabled  --image_in samples/298.jpg --image_out sample/222.jpg  --horizontal_ratio 0.7 --vertical_ratio 0.7
run with video or webcam 
python3 soha.py --video enabled --stream_in samples/stream_in.mp4 --stream_out samples/aa.mp4 --horizontal_ratio 0.7 --vertical_ratio 0.7
...
class SocialDistancing:
    ...
    def trace(self, image, skeletal_coordinates, draw_ellipse_requirements, is_skeletal_overlapped,show_dis_from_cam):
        ...
        for skeletal_coordinate in skeletal_coordinates[0]:
            ...
            cv2.putText(image, ""+str(show_dis_from_cam[i][3])+"", (int(show_dis_from_cam[i][1]),int(show_dis_from_cam[i][2])),cv2.FONT_HERSHEY_SIMPLEX, 1.0, (255, 255,0), 2)
            # Trace skelethon
    ...
    def create_joint_array(self, skeletal_coordinates,inference_box,src_size):
        # Get joints sequence
        bodys_sequence = []
        bodys_probability = []
    ...
    def evaluate_ellipses(self, skeletal_coordinates, draw_ellipse_requirements, ellipse_boxes, ellipse_pool,show_dis_from_cam,xgb_model_loaded):
        ...
        for skeletal_coordinate in skeletal_coordinates:
            ...
            #================HAR=========================================
            abc = np.array([skeletal_coordinate])
            ...
            distance_from_cam = [distance_f_camera,bb_center[0],bb_center[1],preds]
    ...
    def distances_evaluate(self, image, background):
        ...
        if type(skeletal_coordinates) is list:
            ...
            skeletal_coordinates = self.create_joint_array(
                skeletal_coordinates,inference_box,src_size)
            ...
            self.evaluate_ellipses(skeletal_coordinates[0],
                                   draw_ellipse_requirements, ellipse_boxes, ellipse_pool, show_dis_from_cam,xgb_model_loaded)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--stream_in", default="samples/stream_in.mp4",
